...
import {
  constants, address, abi, decimals, underlyings, sTokens
} from './constants';
...
export async function supply(
  ...
  if (sTokenName === constants.sETH) {
    options.abi = abi.sEther;
  } else {
    options.abi = abi.sErc20;
  }

  options._strikeProvider = this._provider;
...
export async function redeem(
  ...
  if (!sTokens.includes(sTokenName) || !underlyings.includes(underlyingName)) {
...
/**
 * Borrows an Ethereum asset from the Strike Protocol for the user. The user's
 ...
 *   const trx = await strike.borrow(Strike.BUSD, busdScaledUp, trxOptions);
 * 
 *   console.log('Ethers.js transaction object', trx);
 * 
 * })().catch(console.error);
...
export async function borrow(
  ...
  const parameters = [ amount ];
  trxOptions.abi = sTokenName === constants.sETH ? abi.sEther : abi.sErc20;
 ...
 *   const trx = await strike.repayBorrow(Strike.BUSD, 32, address);
 * 
 *   console.log('Ethers.js transaction object', trx);
 * 
 * })().catch(console.error);
